/*
  ...
  2. После добавления питомца в корзину, необходимо
  вызвать функцию updateCartDisplay (она обновит отображение корзины).
  3. В корзину можно добавить не более 3 питомцев.
   Если пользователь пытается добавить больше, то в messageBox должен появится текст: 'Вы не можете добавить более 3 питомцев'
...
petShop.addEventListener('click', function (event) {
  // Проверяем, что клик был по кнопке питомца
    ...
    // Получаем ID питомца из объекта события
    ...
    if (cart.length < 3) {
      ...
      // Если корзина полна, показываем сообщение в messageBox
      messageBox.textContent = 'Вы не можете добавить более 3 питомцев';
      // messageBox.style.display = 'block'; // Показываем messageBox, если он скрыт
    }
